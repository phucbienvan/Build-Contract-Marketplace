...
contract Marketplace is Ownable {
    ...
    function _updateFeeRate(uint256 feeDecimal_, uint256 feeRate_) internal {
        require(feeRate_ < 10**(feeDecimal + 2), "bad fee rate");
        feeDecimal = feeDecimal_;
        feeRate = feeRate_;
    ...
    function _caculateFee(uint256 orderId_) private view returns (uint256) {
    ...
    function addPaymentToken(address paymentToken_) external onlyOwner {
        ...
        require(_supportedPaymentTokens.add(paymentToken_), "payment token supported");
    ...
    modifier onlySupportedPaymentToken(address paymentToken_) {
        require(isPaymentTokenSupported(paymentToken_), "unsupported payment token");
        _;
    }
    ...
    function cancelOrder(uint256 orderId_) external {
        Order storage _order = orders[orderId_];
        require(_order.buyer == address(0), "buyer must be zero");
        require(_order.seller == _msgSender(), "must be owner");
    ...
    function executeOrder(uint256 orderId_) external {
        ...
        emit OrderMatched(orderId_, address(this), _msgSender(), _order.tokenId, _order.paymentToken, _order.price);
    }
}
